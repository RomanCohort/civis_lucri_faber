...
class PhonologicalLoop(nn.Module):
    ...
    def forward(self, features: torch.Tensor,
        ...
        self.store_buffer.scatter_(1, idx.view(B, 1, 1).expand(B, 1, self.item_dim), features)
        ...
        active_item = self.store_buffer.gather(
            1, active_item_idx.view(B, 1, 1).expand(B, 1, self.item_dim)
        ).squeeze(1)
...
class AuditoryWorkingMemory(nn.Module):
    ...
    def forward(self, cortical_input: torch.Tensor,
              attention_focus: Optional[torch.Tensor] = None,
              step: int = 0) -> Dict:
        """
...
class AuditoryMotorCoupling(nn.Module):
    ...
    def forward(self, auditory_features: torch.Tensor,
              motor_command: Optional[torch.Tensor] = None,
...
class CognitiveAuditoryCortex(nn.Module):
    ...
    def __init__(self, sample_rate: int = 16000,
        ...
        from spiking_auditory_cortex import SpikingAuditoryCortex
