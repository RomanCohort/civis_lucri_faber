...
class HPAAxis(AbstractBrainRegion):
    """
    HPA轴 - 完整的应激反应级联系统
    ...
    region_name: ClassVar[str] = "hpa"

    @classmethod
    def required_keys(cls) -> Set[str]:
        """Keys this region reads from the shared state."""
        return set(["stress_signal", "uncertainty", "circadian_hour",
                    "ne_level", "inflammation", "social_support"])

    @classmethod
    def output_keys(cls) -> Set[str]:
        """Keys this region writes to the shared state."""
        return set(["cortisol_level", "cortisol", "hpa_crh", "hpa_acth",
                    "allostatic_load", "stress_type", "hpa_suppressed"])
