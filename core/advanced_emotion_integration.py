...
try:
    ...
    logger.warning(f"Advanced emotion modules not available: {e}")
...
class IntegratedAdvancedEmotionSystem:
    ...
    def _integrate_emotions(
        ...
        try:
            ...
            # Tensor creation failed or mood_state missing VAD attributes
        ...
        try:
            ...
            # Tensor shape mismatch during social emotion processing
            sources.append(torch.zeros_like(regulated))
            weights[2] = 0.0
        ...
        try:
            ...
            # Tensor shape mismatch during interoceptive signal processing
            sources.append(torch.zeros_like(regulated))
            weights[3] = 0.0
        ...
        try:
            ...
            # Tensor shape mismatch during contagion processing
            sources.append(torch.zeros_like(regulated))
            weights[4] = 0.0
    ...
    def on_emotion_process(self, event) -> dict[str, Any]:
        ...
        try:
            ...
            # PyTorch processing failed or attribute missing on adv_state
    ...
    def on_hibernate_enter(self, event) -> dict[str, Any]:
        ...
        try:
            ...
            if hasattr(emotion_memory, 'consolidate_during_sleep'):
                ...
                logger.info("Memory consolidation during hibernation")
        except AttributeError:
            # emotion_memory missing consolidate_during_sleep method
            pass
...
def test_integration():
    """测试集成系统"""
    logger.info("=" * 60)
    logger.info("Testing Integrated Advanced Emotion System")
    logger.info("=" * 60)

    if not MODULES_AVAILABLE:
        logger.warning("Modules not available, skipping test")
        return
    ...
    logger.info(f"  Current emotion: {result['advanced_state'].current_emotion}")
    logger.info(f"  Mood valence: {result['advanced_state'].mood_valence:.3f}")
    ...
    logger.info(f"  Social emotion: {result['social']}")

    logger.info("\n[3] Testing with observation...")
    observation = torch.randn(4, 64)
    result = system.process(state, observation=observation)
    logger.info(f"  Contagion: {'detected' if 'contagion' in result else 'none'}")

    logger.info("\n[4] Summary...")
    summary = system.get_summary()
    logger.info(f"  History size: {summary['history_size']}")

    logger.info("\n" + "=" * 60)
    logger.info("Advanced emotion system integrated!")
    logger.info("=" * 60)
