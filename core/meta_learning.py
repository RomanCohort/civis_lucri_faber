...
from core.events import DISSONANCE_DETECTED, MEMORY_ADDED
...
class FirstOrderMAML(nn.Module):
    ...
    def _apply_params(
        ...
        """使用自定义参数的前向传播 (FOMAML functional forward)"""
        # 构建该module的参数子集
        prefix = ""
        for name, mod in self.named_modules():
            if mod is module and name:
                prefix = name + "."
                break

        if prefix:
            sub_params = {
                n[len(prefix):]: p for n, p in params.items()
        ...
        try:
            ...
            return functional_call(module, sub_params, (x,))
...
class UncertaintyAwareActiveLearner:
    ...
    def select_query(
        ...
        info_gains: list[float] | None = None
    ) -> tuple[str, float]:
        """选择最优查询
...
class CognitiveDissonanceDetector:
    ...
    def on_memory_added(self, event) -> dict[str, Any] | None:
