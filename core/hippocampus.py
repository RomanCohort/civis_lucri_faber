...
# ══════════════════════════════════════════════════════
# Hippocampal Theta Rhythm (新增)
# 参考: Buzsáki (2002, 2005) - Theta rhythm in hippocampus
# ══════════════════════════════════════════════════════

@dataclass
class ThetaRhythmConfig:
    """Theta rhythm configuration for hippocampal gating.

    Theta rhythm (~6-10 Hz, typically ~8 Hz) gates hippocampus modes:
    - Trough (phase 0-0.5): Encoding new experiences
    - Peak (phase 0.5-1.0): Memory retrieval and replay

    Reference: Buzsáki (2002) - Theta oscillations in the hippocampus
    """
    frequency: float = 8.0          # Hz, typical range 6-10 Hz
    time_step_ms: float = 5.0       # ms per simulation step
    trough_threshold: float = 0.5   # Phase below this = trough (encode)
                                    # Phase above this = peak (retrieve)
...
class Hippocampus(AbstractBrainRegion):
    """
    完整海马体系统

    整合DG + CA3 + CA1 + EC + Sharp-wave Ripple + HC-PFC连接 + Theta节律
    """

    region_name: ClassVar[str] = "hippocampus"
    ...
    def __init__(
        ...
        pfc_dim: int = 64,
        event_bus=None,
        theta_config: ThetaRhythmConfig | None = None,
    ):
        super().__init__()
        ...
        # Theta rhythm configuration (新增)
        self.theta_config = theta_config or ThetaRhythmConfig()
        self._theta_phase: float = 0.0       # Current theta phase [0, 1)
        self._theta_time: float = 0.0        # Accumulated time in seconds
        self._step_count: int = 0            # Total step count
    ...
    def step(self, internal_state: dict[str, Any] = None, **kwargs) -> dict[str, Any]:
        ...
        Advances theta phase and returns theta rhythm state.
        """
        # Advance theta phase
        self._advance_theta_phase()

        result = self.forward(**kwargs) if hasattr(self, 'forward') and callable(getattr(self, 'forward', None)) else {}

        # Add theta rhythm state to result
        result["theta_phase"] = self._theta_phase
        result["theta_mode"] = self.get_theta_mode()
    ...
    def required_keys(cls) -> Set[str]:
        ...
        return set(["state_tensor", "emotion_signal"])

    @classmethod
    def output_keys(cls) -> Set[str]:
        """Keys this region writes to the shared state."""
        return set(["hippo_episode", "hippo_retrieval", "hippo_consolidation",
                    "hippo_place_code", "hippo_novelty", "theta_phase", "theta_mode"])

    def _advance_theta_phase(self) -> None:
        """Advance theta phase based on time step.

        Uses sinusoidal oscillation: phase = sin(2π * freq * t)
        Normalized to [0, 1) range for gating decisions.
        """
        # Convert time step to seconds
        dt_seconds = self.theta_config.time_step_ms / 1000.0
        self._theta_time += dt_seconds
        self._step_count += 1

        # Compute raw phase from sinusoidal oscillation
        # sin(2π * f * t) oscillates between -1 and 1
        raw_oscillation = math.sin(2 * math.pi * self.theta_config.frequency * self._theta_time)

        # Normalize to [0, 1) for phase representation
        # -1 → 0 (trough), +1 → 0.5 (peak), -1 → 1 (back to trough)
        self._theta_phase = (raw_oscillation + 1.0) / 2.0

    def get_theta_mode(self) -> str:
        """Get current theta mode based on phase.

        Returns:
            "encode" if phase < trough_threshold (trough)
            "retrieve" if phase >= trough_threshold (peak)
        """
        return "encode" if self._theta_phase < self.theta_config.trough_threshold else "retrieve"
    ...
    def is_encoding_phase(self) -> bool:
        """Check if currently in encoding (trough) phase."""
        return self._theta_phase < self.theta_config.trough_threshold

    def is_retrieval_phase(self) -> bool:
        """Check if currently in retrieval (peak) phase."""
        return self._theta_phase >= self.theta_config.trough_threshold

    def _handle_memory_encode(self, event) -> dict:
        """Event-driven handler for memory_encode events.

        Theta rhythm gates encoding vs retrieval:
        - Trough phase (encode): Prioritize memory encoding
        - Peak phase (retrieve): Prioritize memory retrieval
        """
        import numpy as _np
        ...
        # Theta-gated behavior
        encoding = None
        encoding_norm = 0.0
        retrieved_avg_reward = 0.0

        if self.is_encoding_phase():
            # Trough phase: Prioritize encoding
            encoding = self.encode_memory(
                state=state_np,
                action=action_str,
                reward=reward_val,
            )
            encoding_norm = float(_np.linalg.norm(encoding))
        else:
            # Peak phase: Prioritize retrieval/replay
            ...
            # Still encode, but with lower priority
            encoding = self.encode_memory(
                state=state_np,
                action=action_str,
                reward=reward_val,
            )
            encoding_norm = float(_np.linalg.norm(encoding)) * 0.5  # Reduced encoding during retrieval phase

        state["hc_memory_count"] = self.state.memory_count
        state["hc_last_encoding_norm"] = encoding_norm
        state["hc_retrieved_avg_reward"] = retrieved_avg_reward

        # Theta rhythm state
        state["theta_phase"] = self._theta_phase
        state["theta_mode"] = self.get_theta_mode()
        ...
        return {
            ...
            "theta_mode": self.get_theta_mode(),
        }
    ...
    def imagine_future(
        ...
        for _ in range(n_steps):
            ...
            try:
                ...
                # PyTorch tensor operation failed: fallback to noise-based prediction
    ...
    def get_summary(self) -> dict:
        ...
        return {
            ...
            'theta_phase': self._theta_phase,
            'theta_mode': self.get_theta_mode(),
            'theta_frequency': self.theta_config.frequency,
        }
