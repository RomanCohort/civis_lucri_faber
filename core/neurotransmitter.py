...
class DopamineSystem(nn.Module):
    ...
    def __init__(
        ...
        # pathway_level: initialized before compute_reward_signal() may reference it
        self.pathway_level = 0.5
...
class NeurotransmitterSystem(AbstractBrainRegion):
    """
    完整神经递质系统
    """

    region_name: ClassVar[str] = "neurotransmitter"
    ...
    def required_keys(cls) -> Set[str]:
        ...
        return set(["expectation", "novelty", "salience", "threat", "urgency"])

    @classmethod
    def output_keys(cls) -> Set[str]:
        """Keys this region writes to the shared state."""
        return set(["nt_dopamine", "nt_serotonin", "nt_acetylcholine",
                    "nt_norepinephrine", "nt_motivation", "nt_arousal",
                    "nt_state", "dopamine"])
    ...
    def step(
        ...
        """
        # 1. 多巴胺
        dop_result = self.dopamine.compute_reward_signal(reward, expectation)
        dop_level = dop_result[0]  # unpack total_level from (total, tonic, phasic)
