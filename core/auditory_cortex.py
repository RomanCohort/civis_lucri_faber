...
class VentralStream(nn.Module):
    ...
    def forward(self, a1_features: torch.Tensor) -> dict:
        ...
        # 取batch中最多的expert (ModuleList不支持tensor索引)
        expert_idx_p = top_p.mode().values.item() if top_p.numel() > 1 else top_p.item()
        phoneme_out = self.experts_phoneme[expert_idx_p](a1_features)

        # 级2: 音节整合
        gate_s = self.gate_syllable(phoneme_out)
        top_s = gate_s.argmax(dim=-1)
        expert_idx_s = top_s.mode().values.item() if top_s.numel() > 1 else top_s.item()
        syllable_out = self.experts_syllable[expert_idx_s](phoneme_out)
        ...
        return {
            ...
            'what': 'identity/word',
            'expert_used': (expert_idx_p, expert_idx_s),
        }
...
class DorsalStream(nn.Module):
    ...
    def forward(self, a1_features: torch.Tensor) -> dict:
        ...
        expert_idx = top_idx.mode().values.item() if top_idx.numel() > 1 else top_idx.item()
        expert_out = self.experts[expert_idx](a1_features)
