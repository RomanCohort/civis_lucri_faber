...
class BasilarMembrane(nn.Module):
    ...
    def __init__(self, params: CochleaParams):
        ...
        # 每个位置的带宽 (Q值 ~10)
        self.Q = 10.0
        self.register_buffer('bandwidths', torch.tensor(freqs / self.Q))

        # 延迟参数 (ms) - 低频延迟更长
        self.register_buffer('delays_ms', torch.tensor(12.0 / (freqs / 1000)))
    ...
    def forward(self, audio: torch.Tensor) -> dict:
        ...
        F_ = stft.shape[-1]
        freqs = torch.fft.rfftfreq(512, 1.0/self.p.sample_rate, device=audio.device)[:F_]
...
class OuterHairCellAmplifier(nn.Module):
    ...
    def amplify(self, displacement: torch.Tensor,
              feedback: torch.Tensor | None = None) -> torch.Tensor:
        """
        主动放大
        ...
        # 饱和限制
        amplified = torch.tanh(amplified / self.saturation_level) * self.saturation_level
...
class CochleaModel(nn.Module):
    ...
    def __init__(self, params: CochleaParams | None = None):
        super().__init__()
...
class SpikingAuditoryEncoder(nn.Module):
    ...
    def __init__(self, n_channels: int = 128, params: LIFParams | None = None):
        super().__init__()
...
class SpikingAuditoryCortex(nn.Module):
    ...
    def forward(self, audio_left: torch.Tensor,
               audio_right: torch.Tensor | None = None) -> dict:
        """
        audio_left: [B, T]
