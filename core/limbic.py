...
from typing import ClassVar, Set, Any, Dict
...
class ThalamicRelay(nn.Module):
    ...
    def __init__(
        ...
        # Sensory projection for reticular nucleus (pre-allocated, not created in forward())
        self.sensory_proj = nn.Linear(input_dim, n_senses)
...
class LimbicSystem(AbstractBrainRegion):
    """
    边缘系统整合
    ...
    region_name: ClassVar[str] = "limbic"
    ...
    def required_keys(cls) -> Set[str]:
        ...
        return set(["state_tensor", "hippo_retrieval", "dopamine_level"])

    @classmethod
    def output_keys(cls) -> Set[str]:
        """Keys this region writes to the shared state."""
        return set(["limbic_emotion", "limbic_arousal", "limbic_valence",
