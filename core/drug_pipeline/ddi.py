...
class DDIPairRecord:
    ...
    affected_cyp: Optional[str] = None
    ke_modifier_a: float = 1.0   # drug_a的ke修正倍率 (<1=清除减慢)
    ke_modifier_b: float = 1.0   # drug_b的ke修正倍率
    pd_interaction_model: Optional[str] = None  # "bliss" / "synergistic" / "antagonistic" / "additive"
...
def assess_ddi(
    drug_ids: List[str],
    drug_defs: Optional[Dict[str, Any]] = None,
    drug_admet: Optional[Dict[str, Dict]] = None,
) -> DDIResult:
    """评估多药DDI。
...
def _get_clinical_override(drug_a: str, drug_b: str) -> Optional[Dict]:
    """查找临床PD交互覆盖。"""
    key1 = (drug_a, drug_b)
    key2 = (drug_b, drug_a)
...
def combine_pd_deltas(
    ...
    ddi_result: Optional[DDIResult] = None,
