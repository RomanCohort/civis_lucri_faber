...
class ProceduralMemory:
    ...
    def lookup(self, context: np.ndarray) -> ProceduralSkill | None:
        """查找匹配的技能"""
        context_hash = self._hash_context(context)
        best = None
    ...
    def execute_auto(self, context: np.ndarray) -> tuple[int, np.ndarray] | None:
        """自动执行已存档技能
...
class CerebelloSpinalCoordination(nn.Module):
    ...
    def forward(
        ...
        # 2. 脊髓执行
        motor_cmd_np = motor_cmd.detach().cpu().numpy()
        reflex_input = None
