...
from simulacrum.core.auditory_cortex import AuditoryCortex
from simulacrum.core.binaural_auditory import BinauralProcessor
from simulacrum.core.cognitive_auditory_cortex import CognitiveAuditoryCortex
from simulacrum.core.event_auditory_cortex import EventDrivenAuditoryCortex
from simulacrum.core.event_binaural_auditory import EventDrivenBinaural
from simulacrum.core.event_phonetic_perception import EventDrivenPhoneticPerception
from simulacrum.core.phonetic_perception import PhoneticPerception
from simulacrum.core.spiking_auditory_cortex import SpikingAuditoryCortex
from simulacrum.core.formant_synthesis import FormantToWaveform

# ===== 情绪扩展系统 (Emotion Extensions) =====
from simulacrum.core.emergent_emotion import EmergentEmotion
...
from simulacrum.core.cross_modal_binding import CrossModalBinder
from simulacrum.core.multimodal_perception import MultimodalPerception
from simulacrum.core.neuromodulation_integration import NeuromodulationIntegration
from simulacrum.core.snn_core import LeakyIntegrateAndFire, SpikingLayer

# ===== 治疗/药理学系统 (Therapy/Pharmacology) =====
from simulacrum.core.pharmacotherapy_synergy import SynergyCalculator
from simulacrum.core.psychotherapy import PsychotherapySystem
from simulacrum.core.psychopharmacology_sandbox import PsychopharmacologySandbox
from simulacrum.core.therapeutic_experiment import TherapeuticExperiment
from simulacrum.core.psychometric_indicators import PsychometricIndicatorTracker
from simulacrum.core.pd_target_mapping import build_pd_targets
from simulacrum.core.state_key_mapping import UnifiedStateMapping

# ===== 评估系统 (Evaluation) =====
from simulacrum.core.llm_evaluator import LLMEvaluator
...
from simulacrum.core.rhythm import RhythmSystem
...
class Simulacrum:
    ...
    def __init__(
        ...
        print("[OK] Auditory System initialized (9 modules: cortex, binaural, phonetic, spiking, formant)")
        ...
        print("[OK] Emotion Extension System initialized (6 modules: emergent, dynamics, consolidation, regulation, contagion, social)")
        ...
        print("[OK] Cognitive Extension System initialized (5 modules: visual, cross-modal, multimodal, neuromod, SNN)")
        ...
        print("[OK] Therapy/Pharmacology System initialized (7 modules: synergy, psychotherapy, sandbox, experiment, psychometric, PD targets, state mapping)")

        # ===== 维度22: 评估系统 (Evaluation) =====
        self.llm_evaluator = LLMEvaluator(api_client=self.api_client)
        print("[OK] LLM Evaluator initialized")
        ...
        print("[OK] Utility Systems initialized (interference forgetting, policy learning, rhythm)")
    ...
    def step(self, user_input: str = None, user_sentiment: float = 0.0, external_stimulus: float = 0.0, video: torch.Tensor = None) -> AgentState:
        ...
        if self.advanced_emotion is not None:
            ...
            try:
                ...
                self._internal_state['emergent_emotion'] = emergent_out['emotion'][0].detach().tolist()
                self._internal_state['emergent_urgency'] = float(emergent_out.get('urgency', torch.tensor(0.0)))
                ...
                self._internal_state['regulated_emotion'] = regulated_out['regulated_emotion'][0].detach().tolist()
                self._internal_state['emotion_regulation_strategy'] = regulated_out.get('strategy', 'reappraisal')
                ...
                self._internal_state['contagion_emotion'] = contagion_out['final_emotion'][0].detach().tolist()

                # 社会情绪 (羞耻/内疚/自豪/嫉妒等)
                social_emo_out = self.social_emotions(
                    self_state=real_state_t,
                )
                self._internal_state['social_emotions'] = social_emo_out['social_emotions'][0].detach().tolist()
                self._internal_state['dominant_social_emotion'] = social_emo_out.get('dominant_emotion', 'neutral')
            ...
            try:
                ...
                self._internal_state['crossmodal_unified'] = crossmodal_out['unified_repr'][0].detach().tolist()[:10]
                ...
                self._internal_state['nm_dopamine'] = nm_out.get('dopamine', 0.5)
                self._internal_state['nm_serotonin'] = nm_out.get('serotonin', 0.5)
                self._internal_state['nm_acetylcholine'] = nm_out.get('acetylcholine', 0.5)
                self._internal_state['nm_norepinephrine'] = nm_out.get('gamma', 0.5)  # NE/gamma
