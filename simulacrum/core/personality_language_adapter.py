...
class PersonalityLanguageAdapter:
    ...
    def update_from_personality(
        self,
        tripartite: object | None = None,
        identity_core: object | None = None,
    ...
    def get_style_modifiers(self, config: PersonalityStyleConfig | None = None) -> dict:
        """
        获取风格修饰符（用于语言生成）
    ...
    def generate_style_prompt(self, config: PersonalityStyleConfig | None = None) -> str:
        """
        生成风格指令提示词（注入到系统提示词）
    ...
    def get_lexical_bias(self, config: PersonalityStyleConfig | None = None) -> dict:
        """
    ...
    def apply_to_text(self, text: str, config: PersonalityStyleConfig | None = None) -> str:
        """
        将人格风格应用到文本（后处理）
