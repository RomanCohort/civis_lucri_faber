...
class ArticulatoryPlanner(nn.Module):
    ...
    def forward(
        ...
        respiratory_phase: torch.Tensor | None = None,  # [B] 呼吸相位
        auditory_feedback: torch.Tensor | None = None,  # [B, 64] 听觉反馈
        dt: float = 0.01,
    ) -> dict:
        """
        生成发音器运动轨迹
...
class SpeechProductionPipeline(nn.Module):
    ...
    def forward(
        ...
        language_features: torch.Tensor | None = None,  # [B, 64]
        respiratory_phase: torch.Tensor | None = None,   # [B]
        auditory_feedback: torch.Tensor | None = None,   # [B, 64]
        arousal: float = 0.5,
        dt: float = 0.01,
    ) -> dict:
        """
...
class VocalCortex(nn.Module):
    ...
    def forward(
        ...
        emotion_vector: torch.Tensor | None = None,  # [B, 8]
        auditory_feedback: torch.Tensor | None = None,
        arousal: float = 0.5,
        dt: float = 0.01,
