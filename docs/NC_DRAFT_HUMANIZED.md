...
- approximately 23% activation sparsity through selective subscription
...
- Seven pathways: cortisol to PFC, DA to exploration, oxytocin to empathy, etc.
...
│  ┌─────────────────────────────────────────────────────┐   │
│  │           Coupling Pathways (7)                      │   │
│  │    Cortisol to PFC  DA to Exploration  Oxytocin to Empathy   │
│  └─────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
```
...
|---------|------------|------|-------|------|
| State-dependent routing | Bio-Gating | No | No | Limited |
| Sparse activation | EventBus | No | No | Partial |
| Neurochemical coupling | 7 pathways | No | No | Limited |
| Emergent behaviors | Demonstrated | Hard-coded | Production rules | Limited |
| Mathematical formulation | Proofs | No | No | No |
...
**Remark 2.2.2 (indirect emotion/mood influence)**: While Corollary 2.2.1 shows uniform emotion/mood modulation does not directly shift gate probabilities, these states influence routing indirectly through: (1) their effect on neurotransmitter dynamics through coupling pathways (Section 4, P1-P7), for example, stress (high arousal, negative valence) triggers cortisol release which modulates DA through P1 to P2 cascade; and (2) their role in event emission, where high arousal triggers STRESS_EVENT, altering which regions activate through EventBus subscriptions.
...
For $n = 512$ and typical $n_e = 4$, theoretical speedup is $512/4 = 128\times$. Actual speedup is lower due to modulation overhead, achieving approximately 100 times in practice. When $n_e$ is absorbed into the implicit constant (informal context), this simplifies to $O(n)$.
...
Bio-Gating achieves **86.9% FLOP reduction** compared to Standard MoE Top-2. The FLOP reduction stems primarily from Top-1 vs Top-2 expert selection (same mechanism as Switch Transformer). Bio-Gating's computational overhead is the modulation computation: 4,096 FLOPs (approximately 3% of base). Bio-Gating's **novel contribution** is state-dependent expert selection (routing varies with DA/5-HT/NE/VAD/mood), which is absent from content-only MoE architectures.
...
Bio-Gating maintains consistent approximately 50% savings at all scales; modulation overhead becomes negligible relative to expert computation at larger scales.
...
**Emotion modulation strength $\alpha$**:
...
**Neurotransmitter modulation coefficients $\eta, \kappa, \lambda, \beta$**:
...
**Mood coefficient $\beta$**:
$$\frac{\partial g_j}{\partial \beta} = \text{mood} \cdot g_j \cdot (1 - g_j)$$

Since mood applies uniformly to all experts, this gradient is theoretically zero for gate shift (Corollary 2.2.1), but $\beta$ affects training dynamics through the loss landscape.

**Decay factor $\gamma$ (membrane potential)**:
...
**Training stability**: Modulation factors act as multiplicative biases on softmax inputs, avoiding gradient instability from direct softmax manipulation. For $\eta=0.2$, gradient magnitude is approximately 0.05 times gate variance, within stable optimization range.
...
The brain achieves remarkable energy efficiency through sparse activation: only approximately 1-5% of neurons fire at any moment (Lennie, 2003). This sparsity emerges from two mechanisms:
...
|--------|----------|----------------|------------------|------------------|
| HPA Axis | Stress response | cortisol, stress_reactivity | cortisol in [0,1], reactivity = 0.6 | Hypothalamic-Pituitary-Adrenal axis |
| Amygdala | Emotion processing | VAD states | (V,A,D) in [-1,1]³ | Valence-Arousal-Dominance model |
| Hippocampus | Episodic binding | relational_capacity | binding_slots = 4 | Relational memory (Eichenbaum 2017) |
| Prefrontal Cortex | Working memory | WM_capacity, inhibition_rate | capacity = 7, rate in [0,1] | Miller's law; Executive function |
| Basal Ganglia | Action selection | DA-dependent | DA in [0,1] | Dopaminergic gating |
| Thalamus | Sensory gating | attention_gate | gate in [0,1] | Thalamic relay model |
...
| Thermodynamics | Resource management | balance, compute_cost | balance in [-1,1] | Energy homeostasis |
| Metabolic Budget | Energy allocation | resource_budget | budget in [0,100] | Metabolic constraints |
| Sleep System | Memory consolidation | NREM3 gating | NREM3 threshold = 0.8 | Sleep stages |
| Social Cognition | Empathy, ToM | resonance_baseline | resonance in [0,1] | Theory of Mind |

**Total parameters**: approximately 150 across all regions (excluding neural network weights).
...
where $\mathbb{1}$ is the indicator function. Region $r$ activates if and only if $e \in S(r)$.
...
|--------|--------|---------|--------|----------|--------|---------|-----|-------|--------|-------|------|--------|-------|-----|------|-------|-----|--------|
| HPA | Yes | Yes | No | No | No | No | Yes | No | No | Yes | Yes | No | No | Yes | Yes | No | No | No |
| Amygdala | Yes | Yes | Yes | No | No | Yes | Yes | No | Yes | No | No | No | No | No | No | No | Yes | Yes |
| Hippocampus | No | No | Yes | Yes | No | Yes | No | Yes | No | No | No | No | Yes | Yes | No | Yes | Yes | No |
| PFC | Yes | Yes | Yes | Yes | Yes | Yes | Yes | No | Yes | Yes | Yes | Yes | No | Yes | Yes | Yes | Yes | Yes |
| Basal Ganglia | No | No | No | Yes | Yes | No | Yes | No | No | Yes | No | No | No | No | No | Yes | No | Yes |
| Thalamus | No | No | No | No | No | Yes | Yes | No | No | No | No | Yes | No | No | No | No | Yes | No |
| Auditory | No | No | Yes | No | No | Yes | No | No | No | No | No | No | No | No | No | No | Yes | No |
| Visual | No | No | Yes | No | No | Yes | No | No | No | No | No | No | No | No | No | No | Yes | No |
| Glial | No | No | No | No | No | No | No | Yes | No | No | Yes | No | Yes | No | Yes | No | No | No |
| NT Module | Yes | Yes | No | No | No | No | Yes | No | No | Yes | No | No | No | No | Yes | No | No | Yes |
| Thermodynamics | No | No | No | No | No | No | No | Yes | No | No | Yes | Yes | No | No | Yes | No | No | No |
| Metabolic | Yes | No | No | Yes | Yes | No | Yes | No | No | No | Yes | Yes | Yes | No | Yes | No | No | No |
| Sleep | No | No | Yes | No | No | No | No | Yes | No | No | Yes | No | Yes | Yes | No | Yes | No | No |
| Social | No | Yes | Yes | Yes | No | Yes | Yes | No | Yes | No | No | No | No | No | No | No | Yes | Yes |
...
|--------|-------|---------------------|-----------------|
| Mean active regions | 3.2 plus/minus 1.1 | Sparse firing | 1-5% (Lennie 2003) |
...
**Sparsity comparison**: EventBus achieves 23% activation sparsity, which is 5-20 times higher than biological sparse firing rates (1-5%). This reflects a design tradeoff: stricter sparsity would reduce functional coverage. The 23% rate ensures all 14 modules can potentially participate in relevant computations while still achieving significant computational savings compared with dense activation (100%).
...
|--------|-------------------|------------------------|
| PFC | 8 event types | approximately 0.35 |
| Amygdala | 5 event types | approximately 0.22 |
| HPA Axis | 3 event types | approximately 0.13 |

Average activation: approximately 3.2 regions per cycle (23% of 14 regions), approximating biological sparse activation.
...
|----|---------|-----------|-----------|---------------------|
| P1 | Cortisol to PFC | Stress inhibits executive function | Arnsten 2009 | PFC.inhibition_rate *= (1 - cortisol * alpha) |
| P2 | DA to Exploration | Dopamine drives exploration-exploitation | Schultz 2007 | exploration_rate = f(DA, baseline) |
| P3 | Oxytocin to Empathy | Social bonding hormone | Dunbar 2009 | resonance_baseline *= (1 + oxytocin * gamma) |
| P4 | ACh to WM Gate | Attention modulates working memory | Hasselmo 1999 | wm_update_rate = gate(ACh) |
| P5 | GABA to Inhibition | E/I balance control | gamma-aminobutyric acid theory | network_activity = clamp(x, GABA_threshold) |
| P6 | 5-HT to Stability | Serotonin promotes behavioral consistency | Dayan 2009 | decision_variance *= (1 - 5-HT * delta) |
| P7 | NE to Arousal | Norepinephrine boosts salience | Aston-Jones 2005 | attention_gain = SNR_boost(NE) |

### 4.3 Mathematical formulation

**P1: Cortisol to PFC inhibition**
...
**P2: DA to exploration trade-off**
...
**P3: Oxytocin to empathy enhancement**

$$\text{resonance} = \text{resonance}_{\text{baseline}} \cdot e^{\gamma \cdot \text{oxytocin}}$$

where $\gamma = 0.5$ yields approximately 60% resonance boost at maximal oxytocin (oxytocin=1.0).
...
where $\lambda_i \in (0, 1)$ are decay rates and $-\alpha$ captures P1 (cortisol to DA inhibition through stress-induced anhedonia).

**Lemma 4.3 (Stability condition)**: The coupled system is stable if and only if all eigenvalues of $(I + A)$ have magnitude less than 1.
...
Key interactions:
- $\alpha_{12} = \alpha_{21} = 0.3$: P1 and P2 (stress reduces DA through anhedonia cascade; reciprocal DA to stress pathway)
- $\alpha_{36} = \alpha_{63} = 0.2$: P3 and P6 (oxytocin-5-HT synergy for stable bonding)
- $\alpha_{47} = \alpha_{74} = 0.15$: P4 and P7 (ACh-NE attention-arousal synergy)

**Theorem 4.7 (Interaction stability)**: The interaction system is stable if and only if the decay-adjusted system matrix satisfies:
...
**Eigenvalue analysis**: For the 2 by 2 block corresponding to P1 and P2 interaction:
...
|---------|-------------|--------|-------------------|---------------|
| P1 (alpha) | 0.4 | Arnsten 2009, stress-PFC coupling | [0.2, 0.6] | 3-4 times inter-individual variability in stress sensitivity |
| P2 (eta) | 0.2 | Cools et al. 2011, DA-exploration | [0.1, 0.4] | DA behavioral effects vary 2-3 times across individuals |
| P3 (gamma) | 0.5 | Dunbar 2009, oxytocin-ToM | [0.2, 0.8] | Oxytocin effects highly context-dependent; meta-analyses show wide variance |
...
- Stress-PFC coupling varies 3-4 times between resilient and vulnerable individuals
...
|------------|-----------|-----------|-----------|---------------------|
| D2 inverted-U | eta in [0.1, 0.4] | eta=0.1 (weak DA modulation) | eta=0.4 (strong modulation) | Inverted-U shape preserved; optimal occupancy shifts from 65% to 80% |
| Stress anhedonia | alpha in [0.2, 0.6] | alpha=0.2 (resilient individuals) | alpha=0.6 (vulnerable individuals) | PFC decline magnitude scales with alpha; partial recovery trajectory preserved |
| Stockholm bonding | gamma in [0.2, 0.8] | gamma=0.2 (weak oxytocin effect) | gamma=0.8 (strong effect) | Bonding window preserved; peak bonding score scales from 0.45 to 0.92 |

**Key findings**:
1. **Inverted-U shape is robust**: The quadratic therapeutic response persists across all eta values. Optimal occupancy shifts but remains within clinical therapeutic window (60-80%).
2. **Recovery trajectory preserved**: Stress-induced anhedonia shows partial recovery across all alpha values, matching clinical observation that recovery varies but pattern is consistent.
3. **Bonding window emerges reliably**: The stress-inverted-U bonding dynamics persist across gamma values, with quantitative scaling but qualitative pattern preservation.
...
|------------|----------------|------------|--------|
| Exp A (Anhedonia) | P1+P2 cascade | Stress leads to persistent exploration deficit | Confirmed (t=9.15) |
...
**Overhead**: $O(n_e + 9)$ equals negligible for typical $n_e \in [4, 64]$.
...
| Metric | Value |
|--------|-------|
| Mean active regions | 3.2 plus/minus 1.1 |
...
**Qualitative consistency**: All three phenomena demonstrate qualitative pattern matching (inverted-U shape, stress-window effect, gradual recovery). Specific numerical tolerances (plus/minus 5%, plus/minus 0.1) are **not claimed** because:
- Stockholm syndrome has no standardized measurement scale
- Stress anhedonia recovery varies 3-4 times across individuals
...
| 3 | Stress | C3 (P1: cortisol to PFC) | Chronic stress injection | PFC inhibition, DA decline |
...
- 95% confidence intervals through bootstrap
...
**EPS risk model**: Extrapyramidal symptoms follow a **thresholded power law** (reflecting clinical observation that EPS risk is negligible below approximately 75% occupancy and rises sharply above 80%):
...
|-----------|-----------------|-----------|-----------------|
| 30% | 13% plus/minus 4% | 0.00 plus/minus 0.00 | 1.13 plus/minus 0.1 |
| 75% | 33% plus/minus 5% | 0.00 plus/minus 0.00 | **1.33 plus/minus 0.1** |
| 85% | 38% plus/minus 4% | 0.25 plus/minus 0.05 | 1.07 plus/minus 0.1 |
| 95% | 41% plus/minus 3% | 0.64 plus/minus 0.08 | 0.83 plus/minus 0.1 |
...
|-------|--------------|---------------|-------------|------------|
| Resistance | S=0.3 | 0.17 plus/minus 0.05 | 0.76 plus/minus 0.10 | 0.05 plus/minus 0.03 |
| Pressure | S=0.7 | 0.68 plus/minus 0.08 | 0.12 plus/minus 0.05 | 0.78 plus/minus 0.09 |
| Bonding | S=0.85 | 0.88 plus/minus 0.06 | 0.02 plus/minus 0.02 | 0.92 plus/minus 0.06 |
| Overwhelm | S=1.0 | 0.31 plus/minus 0.10 | 0.45 plus/minus 0.12 | 0.15 plus/minus 0.08 |
...
- $\alpha = 0.4$: Cortisol to PFC coupling (hours to days for measurable PFC impairment)
- $\gamma_{\text{regen}} = 0.05$: PFC regeneration rate (weeks for full recovery)
- $\beta = 0.3$: PFC to DA coupling (days to weeks for DA decline)
...
|--------|----------|--------------|--------------------|--------------------|
| Cortisol | 0.45 plus/minus 0.05 | 1.0 plus/minus 0.0 | 0.48 plus/minus 0.06 | 0.45 plus/minus 0.05 |
| PFC inhibition | 0.70 plus/minus 0.03 | 0.42 plus/minus 0.04 | 0.51 plus/minus 0.05 | 0.65 plus/minus 0.04 |
| Exploration | 0.099 plus/minus 0.01 | 0.051 plus/minus 0.02 | 0.068 plus/minus 0.02 | 0.091 plus/minus 0.02 |
| Mesolimbic DA | 0.80 plus/minus 0.05 | 0.45 plus/minus 0.08 | 0.55 plus/minus 0.07 | 0.72 plus/minus 0.06 |
...
- Functional connectivity through subscription patterns
- Scalable O(k·n) complexity

**C3: Coupling pathways**
- Empirically-grounded neuromodulatory interactions
- State-dependent computation through chemical signaling
...
| **State-dependent routing** | Yes: DA/5-HT/NE/VAD/mood | No | No: Fixed productions | No: Fixed productions | Limited (activation) | No |
...
| **Sparsity** | approximately 23% regions active | approximately 50% experts active | Rule-based | Chunk-based | approximately 10% codelets | Continuous |
...
| **Mathematical formulation** | Yes: Proofs (Section 2-4) | Yes: Complexity analysis | Partial (activation equations) | Partial (base-level activation) | Partial | Yes: Neural dynamics |
...
| **Neuromodulation** | Yes: Central mechanism | No | No | No | Limited (arousal) | Limited |
| **Emotional modeling** | Yes: VAD + mood | No | No | No | Simple affect | No |
...
- Mistral 8x7B (Top-2) on language generation
- Measure: accuracy, FLOP efficiency, routing stability

**3. Neural data fitting**: Test pathway dynamics against recordings:
- fMRI BOLD under stress tasks to estimate cortisol to PFC coefficient
- DA neuron firing during reward to test exploration dynamics
- pupillometry under arousal to estimate NE modulation strength
...
|----------|-----------------|---------------------|----------------|
| Intel Loihi 2 | 8-bit (signed) | approximately 1M neurons/chip | Requires event-to-spike transduction with rate/temporal coding |
| IBM TrueNorth | Binary weights | 1M neurons, 256 cores | Fixed compile-time topology incompatible with runtime subscriptions |
| SpiNNaker 2 | 32-bit (ARM-based) | approximately 150K neurons/board | Software-based spiking; event dispatch overhead significant |

**Event-to-spike encoding**: EventBus events would require encoding as:
- Rate-coded spike bursts (event type to spike frequency)
- Temporal patterns (event timing to spike delay coding)
- Population codes (event distributed across neuron ensemble)

This encoding layer is non-trivial and adds latency.

**Subscription semantics mismatch**: EventBus's runtime subscription model conflicts with neuromorphic compile-time connectivity. TrueNorth requires predetermined connection graphs; Loihi supports plasticity but not dynamic subscription changes. The 14-module EventBus architecture would need restructuring, potentially pre-compiling all subscription patterns and selecting at runtime through gating.

**Energy efficiency**: We do not claim specific energy savings. Event-driven sparsity principle aligns with neuromorphic energy efficiency design, but quantitative savings depend on: (1) platform-specific power profiles (Loihi: approximately 0.05-0.5 mW/core idle [Davies et al., 2018]; TrueNorth: approximately 70 mW total regardless of activation [Merolla et al., 2014]), (2) spike routing overhead, (3) event dispatch implementation. **Critical metrics not measured**: energy per event (mJ), spike-to-event transduction latency (microseconds), and event throughput (events/s) remain undetermined; these require hardware deployment beyond this algorithmic contribution.
...
We presented Bio-Gating, a neurotransmitter-modulated routing mechanism that extends Mixture-of-Experts architectures with neuromodulatory state dependence. The computational contributions are: (1) a mathematically rigorous formulation of state-dependent gating with stability and convergence guarantees; (2) an event-driven sparse activation architecture achieving approximately 23% activation sparsity; and (3) seven empirically-grounded coupling pathways with formalized dynamics.

Neuromodulatory dynamics are a computational primitive: routing and activation should reflect the neurochemical state of the system. This property is present in biological computation but absent from standard neural architectures. Behavioral demonstrations show that Bio-Gating produces state-dependent phenomena (D2 inverted-U response, stress-scar anhedonia) that emerge from the interaction of neuromodulation with routing dynamics, not from explicit programming.
...
2. **EventBus** implements event-driven sparse activation through publish-subscribe, achieving approximately 23% activation sparsity with O(k·n) complexity versus O(n²·d) for attention-based systems.
...
Neuromodulatory state shapes routing, a property present in biological cognition but absent from standard neural architectures. This enables adaptive AI systems whose behavior varies with internal state in biologically interpretable ways.
...
Farde, L., Nordstrom, A. L., Wiesel, F. A., Halldin, C., Sedvall, G., & Uppfeldt, G. (1992). Positron emission tomographic analysis of central D1 and D2 dopamine receptor occupancy in patients treated with classical neuroleptics and clozapine. Archives of General Psychiatry, 49(7), 538-544.
...
*Word count: approximately 7,500 (main text)*
