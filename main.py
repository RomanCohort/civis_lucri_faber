...
def main():
    ...
    logger.info(f"  - Compute Cost/sec: {config.compute_cost_per_sec}")
    logger.info(f"  - Intrinsic Motivation Lambda: {config.intrinsic_motivation_lambda}")
    logger.info(f"  - Alignment Check Interval: {config.alignment_check_interval}")
    logger.info(f"  - API Client: {'Configured' if config.openai_api_key or config.anthropic_api_key else 'Mock Mode'}")
